...
def Estimate(Keypoints_x,Keypoints_y,Keypoints_x_curr,Keypoints_y_curr):

    estimation_x = np.array([0])
    estimation_y = np.array([0])
    for i in range(0,len(Keypoints_x)):
        esti = Keypoints_x[i] - Keypoints_x_curr[i]
        estimation_x = np.vstack([estimation_x, esti])
        esti = Keypoints_y[i] - Keypoints_y_curr[i]
        estimation_y = np.vstack([estimation_y, esti])

    estimation_x = np.delete(detection, (0), axis=0)
    estimation_y = np.delete(detection, (0), axis=0)
...
def __main__(scheme , image = 0):
    ...
    #---------------------------------------------------------------------------------------------------------------------------
        ...
        return cengtroids_x, cengtroids_y
